...
export default function ProfilePage() {
  ...
  return (
    <main className="p-6 max-w-3xl mx-auto">
      ...
      {/* 🧘 Practice Quest History with Delete */}
      <section>
        ...
        {questsData?.allPracticeQuests.length === 0 && (
          <p className="text-gray-500">No quests yet.</p>
        )}
        <ul className="space-y-2">
          {questsData?.allPracticeQuests.map((quest) => (
            <li key={quest.id} className="border rounded p-2 flex justify-between items-center">
              <span>
                ...
                {quest.completed && <span className="ml-2 text-green-600 font-bold">✅ Completed</span>}
              </span>
              <button
                className="text-red-500 hover:text-red-700 ml-4 disabled:opacity-50"
                onClick={async () => {
