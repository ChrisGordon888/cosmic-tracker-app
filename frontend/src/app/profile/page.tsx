...
// Optional: reusable types
type SacredYesEntry = { id: string; date: string; text: string };
type MoodEntry = { id: string; date: string; mood: string; note?: string };
type PracticeQuest = {
  id: string;
  date: string;
  name: string;
  ...
  return (
    <main className="profile-page min-h-screen flex flex-col items-center justify-start p-6 relative overflow-hidden">
      ...
      <section className="mb-12">
        ...
        {sacredYesData?.allSacredYes.length === 0 && <p>No Sacred Yes entries yet.</p>}

        <ul className="space-y-4">
          {sacredYesData?.allSacredYes.map((entry: SacredYesEntry) => (
      ...
      <section className="mb-12">
        ...
        {moodData?.allMoodEntries.length === 0 && <p className="text-gray-500">No mood entries yet.</p>}

        <ul className="space-y-4">
          {moodData?.allMoodEntries.map((entry: MoodEntry) => (
      ...
      <section>
        ...
        {questsData?.allPracticeQuests.filter((q: PracticeQuest) => q.completedReps >= q.repetitions).length === 0 && (
          <p>No completed quests yet.</p>
        )}

        <ul className="space-y-4">
          {questsData?.allPracticeQuests
            .filter((quest: PracticeQuest) => quest.completedReps >= quest.repetitions)
            .map((quest: PracticeQuest) => (
