...
type ReviewItem = {
    track: {
        ...
        realmFinderAlignment?: number | null; realmFinderSignals?: string[] | null;
        realmFinderSummary?: string | null; realmFinderVersion?: string | null;
    };
                            ...
                            <div className="mt-5 grid gap-3 md:grid-cols-2">
                                ...
                                <div className="rounded-2xl border border-violet-300/15 bg-violet-300/[0.035] p-4">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        <p className="text-[10px] uppercase tracking-[0.18em] text-violet-200/65">Realm Finder</p>
                                        {track.realmFinderAlignment != null && (
                                            <span className="rounded-full border border-violet-200/15 px-2.5 py-1 text-[10px] text-violet-100/70">
                                                {track.realmFinderAlignment}% quiz alignment
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    {track.realmFinderSuggestedRealmId != null ? (
                                        <>
                                            <strong className="mt-2 block text-sm text-white/90">
                                                {REALMS.find((realm) => realm.id === track.realmFinderSuggestedRealmId)?.label || `Realm ${track.realmFinderSuggestedRealmId}`}
                                            </strong>
                                            {track.realmFinderSecondaryRealmId != null && (
                                                <p className="mt-1 text-xs text-white/45">
                                                    Secondary pull: {REALMS.find((realm) => realm.id === track.realmFinderSecondaryRealmId)?.label || `Realm ${track.realmFinderSecondaryRealmId}`}
                                                </p>
                                            )}
                                            {track.realmFinderSummary && (
                                                <p className="mt-3 text-xs leading-5 text-white/50">{track.realmFinderSummary}</p>
                                            )}
                                            {!!track.realmFinderSignals?.length && (
                                                <div className="mt-3 flex flex-wrap gap-1.5">
                                                    {track.realmFinderSignals.map((signal) => (
                                                        <span key={signal} className="rounded-full border border-violet-200/10 bg-violet-200/[0.035] px-2.5 py-1 text-[10px] text-violet-100/65">
                                                            {signal}
                                                        </span>
                                                    ))}
                                                </div>
