...
export default function AddPracticeQuestForm() {
  ...
  return (
    <section className="mb-8 p-4 rounded-lg border border-green-200 dark:border-green-700 bg-white dark:bg-gray-800 max-w-xl w-full text-left">
      <h2 className="text-xl font-semibold mb-4">➕ Add Practice Quest</h2>

      {ritualsLoading && <p>Loading rituals...</p>}
      {ritualsError && <p className="text-red-600">Error: {ritualsError.message}</p>}
      {message && (
        <p
          className={`text-sm font-semibold mb-2 animate-pulse ${
            message.startsWith("🎉")
